...
# To be used with Propublica's COMPAS dataset
...
def calculate_fairness_scores(m_y1, m_predictedvalues, m_predictedprobs, f_y1, f_predictedvalues, f_predictedprobs):
    ...
    print('Caucasian Accuracy', (m_tn + m_tp)/len(m_predictedvalues))
    print('African American Accuracy', (f_tn + f_tp)/len(f_predictedvalues))

    
    # Print Confusion Matrix
    print('True Negative', m_tn + f_tn, 'C:', m_tn, 'A:', f_tn)
    print('False Positive', m_fp + f_fp, 'C:', m_fp, 'A:', f_fp)
    print('False Negative', m_fn + f_fn, 'C:', m_fn, 'A:', f_fn)
    print('True Positive', m_tp + f_tp, 'C:', m_tp, 'A:', f_tp)
    ...
    # Brier Scores
    # Male Brier Score
    fairness_dict['Brier_C'] = brier_score(m_y1, m_predictedprobs)
    
    # Female Brier Score
    fairness_dict['Brier_A'] = brier_score(f_y1, f_predictedprobs)
...
def main():
    ...
    if userinput == 3 or userinput == 4:
        ...
        # should be compasprepout_Caucasian.csv and compasprepout_African_American.csv
        c_file = "compasprepout_Caucasian.csv"
        a_file = "compasprepout_African_American.csv"
        
        # load the datasets with first row as column names
        # dataset has 0 - did not recidivate and 1 - did recidivate
        c_dataframe = read_csv(c_file, header=0)
        a_dataframe = read_csv(a_file, header=0)  
    
        col_names = list(c_dataframe.columns.values) # both races have same attributes
        col_names.remove('race_Caucasian') # not using race when training split models
        
        
        # make a dataframe for each race that does not contain the class values
        # also takes race out of dataframes
        # note that one-hot encoding and min-max scaling have already been
        # implemented on categorical and numeric attributes, respectively
        c_X1 = c_dataframe[col_names[0:-1]].to_numpy()
        a_X1 = a_dataframe[col_names[0:-1]].to_numpy()
        
        # get the class values
        c_y1 = c_dataframe['two_year_recid']
        a_y1 = a_dataframe['two_year_recid']
        
        if userinput == 3: # using cross_validate method - does automatically on each fold
            c_scores, c_coefsums = cross_validate_method(c_X1, c_y1)
            get_model_coefficients(c_X1, col_names, c_coefsums, c_scores)
            a_scores, a_coefsums = cross_validate_method(a_X1, a_y1)
            get_model_coefficients(a_X1, col_names, a_coefsums, a_scores)
        
        else: # userinput is 4
            c_scores, c_coefsums, c_predictedvalues, c_predictedprobs, c_int = manual_cross_validation(c_X1, c_y1, col_names)
            a_scores, a_coefsums, a_predictedvalues, a_predictedprobs, a_int = manual_cross_validation(a_X1, a_y1, col_names)
    
            print('Coefficients For Caucasians:')
            get_model_coefficients(c_X1, col_names, c_coefsums, c_scores)
            print('Coefficients For African Americans:')
            get_model_coefficients(a_X1, col_names, a_coefsums, a_scores)
            
            fairness_dict = calculate_fairness_scores(c_y1, c_predictedvalues, c_predictedprobs, a_y1, a_predictedvalues, a_predictedprobs)
            
            print(fairness_dict)
            
    else: # userinput  is 1, 2, or 5
        # define the location of the dataset
        # file should be compasprepout.csv
        file = "compasprepout.csv"
        ...
        # dataset has 0 - did not recidivate 1 - did recidivate
        ...
        if userinput == 1: # using cross_validate method - does automatically on each fold
            ...
            if userinput == 5:    
                ...
                new_cols.remove('race_Caucasian')
            
                scores, coefsums, predictedvalues, predictedprobs, intercept = manual_cross_validation_merge(X1, y1, col_names, new_cols, col_names.index('race_Caucasian')) 
            
                # get coefficient averages
            ...
            # A tuple is African-American if race_Caucasian is 0
            a_rows = dataframe[dataframe.race_Caucasian == 0]
            
            # list of actual classification values for African-American tuples
            a_y1 = list(a_rows['two_year_recid'])
            # list of predicted classification values for African-American tuples
            a_predictedvalues = []
            # list of predicted classification probabilities for African-American tuples
            a_predictedprobs = []
            for i in a_rows.index:
                a_predictedvalues.append(predictedvalues[i])
                a_predictedprobs.append(predictedprobs[i])
             
                
            # A tuple is Caucasian if race_Caucasian is 1
            c_rows = dataframe[dataframe.race_Caucasian == 1]
            
            # list of actual classification values for Caucasian tuples
            c_y1 = list(c_rows['two_year_recid'])
            # list of predicted classification values for Caucasian tuples
            c_predictedvalues = []
            # list of predicted classification probabilities for Caucasian tuples 
            c_predictedprobs = []
            for i in c_rows.index:
                c_predictedvalues.append(predictedvalues[i])
                c_predictedprobs.append(predictedprobs[i])
